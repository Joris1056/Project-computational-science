...
def random_search(iterations, n_runs_per_set):
    ...
    for i in range(iterations):
        ...
        params = {
            ...
            'degeneration_p_stage4':np.random.uniform(0.04, 0.2),
            'degeneration_p_stage5':np.random.uniform(0.05, 0.25),
            'lateral_base_multiplier':np.random.uniform(1, 1.5),
            'lateral_ratio_multiplication':np.random.uniform(2, 6),
            'ventral_base_multiplier':np.random.uniform(1, 1.5),
            'ventral_ratio_multiplication':np.random.uniform(2, 6),
...
if __name__ == "__main__":
    best_params, best_RMSE, results, best_time, best_neuron_alive, best_t_70= random_search(1, 1)
